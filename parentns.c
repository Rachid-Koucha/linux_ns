...
Tool which determine the parent of the namespaces of a given process
...
int main(int ac, char *av[])
{
  ...
  for (rc = 0, i = 0; ns[i]; i ++) {
    ...
    // belongs to
    ufd = ioctl(tfd, NS_GET_PARENT);

    if (ufd < 0) {
      ERR("ioctl(%s, NS_GET_PARENT): '%m' (%d)\n", tpath, errno);
      ...
      printf("%s is child of [Device,Inode]: [%lu,%lu]\n", tpath, st.st_dev, st.st_ino);
    }
